smc_abc_dirichlet <- function(N,a,acc_rate_stop,prior_sim,prior_eval,distance_fun,extra_args){
  ...
  while (acc_rate > acc_rate_stop){
    ...
    mle_fit = dirichlet.mle(theta)
    proposal_alpha = mle_fit$alpha
    ...
    for (i in (N_keep+1):N){
      for (r in 1:trial_mcmc_iters){
        ...
        theta_prop = rdirichlet(1, proposal_alpha)
        
        prior_curr = prior_eval(theta[i,], extra_args)
        prior_prop = prior_eval(theta_prop, extra_args)
        
        proposal_prop = ddirichlet(theta_prop, proposal_alpha)
        proposal_curr = ddirichlet(theta[i,], proposal_alpha)
        
        if (runif(1) > prior_prop*proposal_curr/(prior_curr*proposal_prop)){  # early rejection
          next
        }
